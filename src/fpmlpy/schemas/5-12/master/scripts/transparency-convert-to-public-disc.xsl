...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" 
	...
	xmlns:rec="http://www.fpml.org/FpML-5/transparency"
	xmlns:record="http://www.fpml.org/FpML-5/transparency"
	xpath-default-namespace="http://www.fpml.org/FpML-5/transparency"
	...
	<xsl:variable name="target.ns" select="'http://www.fpml.org/FpML-5/transparency'"/>
	...
	<xsl:template match="publicExecutionReport" >
		<!--
		...
		<publicDisclosure fpmlVersion="{$version}" xsi:schemaLocation="http://www.fpml.org/FpML-5/transparency ../../fpml-main-{$version}.xsd http://www.w3.org/2000/09/xmldsig# ../../xmldsig-core-schema.xsd">
			<!--
			<xsl:copy-of select="@*"/>
			-->
	...
	<xsl:template match="trans:reportingRegime" mode="postprocess"/>
	<xsl:template match="trans:category[contains(@categoryScheme, 'dtcc')]" mode="postprocess"/>

	<xsl:template match="trans:tradeHeader" priority="10" mode="postprocess">
		<xsl:copy>
	...
	<!--
	<xsl:template match="trans:payment" priority="10" mode="postprocess">
		<xsl:comment>Post-process payment to get elements in the rigth order.</xsl:comment>
		<xsl:copy>
			<xsl:copy-of select="trans:payerPartyReference"/>
			<xsl:copy-of select="trans:receiverPartyReference"/>
			<xsl:copy-of select="trans:paymentAmount"/>
			<xsl:apply-templates mode="postprocess-payment" select="trans:paymentDate"/>
			<xsl:copy-of select="trans:paymentType"/>
			<xsl:copy-of select="trans:settlementInformation"/>
		</xsl:copy>
	</xsl:template>	
	...
	<xsl:template match="trans:adjustableDate" priority="10" mode="postprocess-payment">
		<xsl:copy-of select="trans:unadjustedDate"/>
		<xsl:copy-of select="trans:dateAdjustments"/>
		<xsl:copy-of select="trans:adjustedDate"/>
	</xsl:template>	
-->

	<xsl:template match="trans:tradeHeader" mode="uti">
		<xsl:variable name="uti" select="trans:partyTradeIdentifier[issuer and not(contains(trans:issuer/@issuerIdScheme, 'cftc') and contains(trans:tradeId/@tradeIdScheme, 'unique-transaction-identifier'))]"/>
		<xsl:if test="$uti">
			<uniqueTransactionIdentifier>
				<xsl:copy-of select="$uti/trans:issuer"/>
				<xsl:copy-of select="$uti/trans:tradeId"/>
			</uniqueTransactionIdentifier>
		</xsl:if>
	</xsl:template>	

	<xsl:template match="trans:tradeHeader" mode="usi">
		<xsl:variable name="usi" select="trans:partyTradeIdentifier[contains(trans:issuer/@issuerIdScheme, 'cftc') and contains(trans:tradeId/@tradeIdScheme, 'unique-transaction-identifier')]"/>
		<xsl:if test="$usi">
			<uniqueSwapIdentifier>
				<xsl:copy-of select="$usi[1]/trans:issuer"/>
				<xsl:copy-of select="$usi[1]/trans:tradeId"/>
			</uniqueSwapIdentifier>
		</xsl:if>
	</xsl:template>	

	<xsl:template match="trans:linkId" mode="postprocess">
		<linkedTrade>
	...
	<xsl:template match="trans:originatingTradeId|trans:blockTradeId|trans:resultingTradeId|trans:allocationTradeId" mode="postprocess">
	...
	<xsl:template match="*" mode="linked-trade">
		...
		<linkedTrade>
			...
			<linkId>
				<xsl:if test="trans:tradeId/@tradeIdScheme">
					<xsl:attribute name="linkIdScheme" select="trans:tradeId/@tradeIdScheme"/>
				</xsl:if>
				<xsl:apply-templates mode="format-link-id" select="."/>
			</linkId>
		</linkedTrade>
	...
	<xsl:template match="*[trans:issuer]" mode="format-link-id" priority="10" >
		<xsl:value-of select="concat(trans:issuer, trans:tradeId)"/>
	</xsl:template>	
	...
	<xsl:template match="@fpmlVersion" priority="10">
		<xsl:attribute name="fpmlVersion"><xsl:value-of select="$version"/></xsl:attribute>
	</xsl:template>	
	...
	<xsl:variable name="templateTF">
		...
		<eventTimestamp field="Trade Execution Time"/>
		<!--
		...
		<regulationSpecificInformation>
			<cftcPart45 copywrapper="//reportingRegime[reportingRole and .//supervisoryBody='CFTC']">
		...
		<quote copy="//quote[not(quoteBasis)]" mode="skipelement"/>
	...
	<xsl:template match="*" mode="evaluate-xpath" >
		...
		<xsl:variable name="xp2" select="replace($xpath, 'rec:publicExecutionReport/', '')"/>
