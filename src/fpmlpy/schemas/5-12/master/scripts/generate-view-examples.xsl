...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:common="http://exslt.org/common" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:view="http://www.fpml.org/views" xmlns:abc="http://www.abc.com/master" xmlns:tr="http://www.sdr.com/regreporting" xmlns:master="http://www.fpml.org/FpML-5/master" exclude-result-prefixes="common xsd xsi view abc tr">
...
<xsl:import href="add-version.xsl"/>
	...
	<xsl:variable name="tr.ns" select="'http://www.sdr.com/regreporting'"/>
	...
	<xsl:template match="*:FpML|*:dataDocument|*:legalDocument|*[@fpmlVersion]" mode="go">
		<!--
		...
		<xsl:element name="{$rootName}" namespace="{$FpML.ns}">
			...
			<xsl:apply-templates mode="schema-loc" select="."/>
			<xsl:if test="not($rootName='dataDocument') and not($rootName='legalDocument') and not(*:header) and $view != 'reporting'">
	...
	<xsl:template match="tr:*" mode="apply-view" priority="5">
		<xsl:element name="tr:{local-name(.)}" namespace="{$tr.ns}">
			<xsl:copy-of select="@*"/>
			<xsl:apply-templates mode="apply-view"/>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template match="*[contains(@xsi:schemaLocation, 'sdr.com')]" mode="schema-loc">
		<xsl:attribute name="xsi:schemaLocation">
			<xsl:value-of select="$tr.ns"/>
			<xsl:text> </xsl:text>
			<xsl:value-of select="$prefix"/>
			<xsl:text>../reg-reporting-example-extension-</xsl:text>
			<xsl:value-of select="$version"/>
			<xsl:text>.xsd </xsl:text>
			<xsl:value-of select="$FpML.ns"/>
			<xsl:text> </xsl:text>
