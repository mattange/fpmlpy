...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" 
	...
	exclude-result-prefixes="common xsd saxon prod trans"
	>
	...
	<xsl:variable name="templateTF">
		<publicReportingProductInfo>
			...
			<otherCalculationTerms>
				<dayCountFraction copy=".//dayCountFraction"/>
				<paymentFrequency copy=".//paymentFrequency"/>
				<!--
				<calculationFrequency copy=".//calculationFrequency"/>
				-->
				<resetFrequency copy=".//resetFrequency"/>
			</otherCalculationTerms>
...
<!-- MiFIR specific -->
	...
	<xsl:template match="trade" priority="10">
		...
		<xsl:copy>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<!--<xsl:message>Applying default rules!!!! </xsl:message>-->
					<xsl:apply-templates mode="default-product-processor" select="."/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:copy>
	...
	<xsl:template match="*" mode="evaluate-xpath" >
		...
		<xsl:variable name="xp2" select="replace($xpath, 'trans:nonpublicExecutionReport/', '')"/>
		<xsl:variable name="xp2b" select="replace($xp2, 'rec:nonpublicExecutionReport/', '')"/>
		<!--
		<xsl:variable name="xp21" select="replace($xp2, 'trans:trade/trans:tradeHeader', 'trans:transactionIdentification')"/> 
		<xsl:variable name="xp3" select="replace($xp21, 'trans:trade/', 'trans:productInformation/')"/>
		-->
		<xsl:variable name="xp3" select="replace($xp2b, 'trans:publicDisclosure/', '')"/>
	...
	<xsl:template name="map-xpath-variables">
		...
		<xsl:variable name="xp0" select="replace($xpath, 'trans:\$reportingPartyRef', '//trans:reportingPartyReference/@href')"/>
		<xsl:variable name="xp1" select="replace($xp0, '$reportingPartyRef', '//trans:reportingPartyReference/@href')"/>
		<xsl:variable name="xp2" select="replace($xp1, 'trans:\$counterPartyRef', '//trans:counterPartyReference/@href')"/>
		<xsl:variable name="xp3" select="replace($xp2, '$counterPartyRef', '//trans:counterPartyReference/@href')"/>
		<xsl:if test="$has">
	...
	<xsl:template match="trade" mode="default-product-processor" priority="10">
		<xsl:comment>Using default rules, no xpaths defined for this product</xsl:comment>
		<xsl:variable name="prod" select="*[2]"/>
		<publicReportingProductInfo>
			<xsl:apply-templates mode="copy-product-classification" select="$prod/*"/>
			<xsl:copy-of select="nonStandardTerms"/>
			<dates>
				<xsl:apply-templates mode="get-effective-date" select="$prod"/>
				<xsl:apply-templates mode="get-termination-date" select="$prod"/>
			</dates>
			<sizes>
				<xsl:apply-templates mode="get-notionals" select="$prod"/>
			</sizes>
			<underlyers>
				<xsl:apply-templates mode="get-underlying-asset" select="$prod"/>
			</underlyers>
		</publicReportingProductInfo>
	</xsl:template>
	...
	<xsl:template mode="get-termination-date" match="*">
		...
		<xsl:choose>
			<xsl:when test="string-length(string($termDates)) &gt; 0">					<!-- when there are termination dates specified -->
				<contractExpirationDate>
					<xsl:value-of select="$termDates[1]"/>			<!-- get the latest -->
				</contractExpirationDate>
			</xsl:when>
			<xsl:when test=".//expirationDate/unadjustedDate">
				<contractExpirationDate>
	...
	<xsl:template mode="get-notionals" match="*">
		...
		<xsl:if test="$notl1">
			<notional>
				<currency><xsl:value-of select="$notl1/currency"/></currency>
				<amount><xsl:value-of select="$notl1/(amount|initialValue)"/></amount>
			</notional>
		</xsl:if>
		<xsl:if test="$notl2">
			<notional>
				<currency><xsl:value-of select="$notl2/currency"/></currency>
				<amount><xsl:value-of select="$notl2/(amount|initialValue)"/></amount>
			</notional>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template mode="get-notls" match="numberOfOptions">
		<notional>
			<amount>
				<xsl:value-of select="format-number(. * ../strike/strikePrice, '#########.00')"/>  <!-- or should it be spot price? -->
