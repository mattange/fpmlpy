...
<xsl:import href="utils.xsl"/>
	...
	<xsl:variable name="notview" select="concat('!', $view)"/>
        ...
        <xsl:template match="documentBase"><![CDATA[var tocTab = new Array();var ir=0;tocTab[ir++] = new Array ("Top", "]]><xsl:value-of select="titleArch"/><xsl:value-of select="title"/><xsl:if test="not(string-length(title) = 0)"> <xsl:call-template name="view.label"/></xsl:if><![CDATA[", "]]><xsl:value-of select="$targetFile"/><![CDATA[");]]>
